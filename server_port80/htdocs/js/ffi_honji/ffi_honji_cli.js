 ...
 *   関数名は f で始まる英数字。
 */
let  a_port = 80;                       // honji_svr の待ち受けポート
const ahnjOK = 0;                       // OK時に返す値
const ahnjNG = 9999;                    // NG時に返す値
...
const ffi_honji_unlock = async function() {
  const xijson = {
    xcmdno:3
  }
  const xretun = await ffi_honji_fetch( xijson);
  if ( xretun != ahnjNG) {
    return ahnjOK;
  } else {
    return ahnjNG;
...
const ffi_honji_fetch = async function( xijson) {
  try {
...
class MyClass{
  ...
  ffi_call = async function( xfuncn, ... xargv) {
    ...
    if ( xretun === ahnjNG) {
      ...
      this.mojson = xretun;   // 退避

      return this.mojson.xreslt;   // 正常リターン
    }
  };
};

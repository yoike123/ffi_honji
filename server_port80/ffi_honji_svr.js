...
// command  1 (execute ffi.library) -> #fcmd01()
// command  2 (call dll function)   -> #fcmd02()
//
// command 11 (create sqlite3 database file)  -> #fcmd11()
// command 12 (create sqlite3 table)          -> #fcmd12()
// command 13 (insert sqlite3 table)          -> #fcmd13()
// command 14 (select sqlite3 table)          -> #fcmd14()
// command 15 (update sqlite3 table)          -> #fcmd15()
// command 16 (delete sqlite3 table)          -> #fcmd16()
...
class THONJI {
    ...
    for ( let xindex = 0; xindex < xarray[1].length; xindex++) {
      if ( typeof xarray[1][xindex] == "string") {
        ...
        if ( typeof xelement0 == "string") {
          ...
          throw new Error("DLL function define parse error.");
        }
      } else {
        throw new Error("DLL function define parse error.");
      }
  ...
  #fcmd01( xijson) {
    ...
    if ( this.#m_lock == 0) {                // ロックがかかっているとき
      let xojson = {};
      Object.assign( xojson, xijson);
      xojson["xstats"] = "LK";
      xojson["xerrno"] = 7;
      xojson["xmesag"] = this.#fermsg( 7);
    ...
    try {
      ...
      let xojson = {
        ...
        xstats: "NG",
        xerrno: 4,
        xmesag: this.#fermsg(4) + e.message
      };
      return JSON.stringify( xojson);
    }
      ...
      for ( let xindex = 0; xindex < xfuncs[1].length; xindex++) {
        if ( typeof xfuncs[1][xindex] == "string") {
          ...
          if ( typeof xelement0 == "string") {
            ...
            throw new Error("xinarg(xcmdno=2) parse error.");
          }
        } else {
          throw new Error("xinarg(xcmdno=2) parse error.");
        }
      ...
      for ( let xindex = 0; xindex < xfuncs[1].length; xindex++) {
        if ( typeof xfuncs[1][xindex] == "string") {
          ...
          if ( typeof xelement0 == "string") {
            ...
            throw new Error("xotarg(xcmdno=2) make error.");
          }
        } else {
          throw new Error("xotarg(xcmdno=2) make error.");
        }
  ...
  /**
   * fcmd14 - command 14 の実装
   * @brief 新規SQLITE３ファイル作成
   * @param[in]   xijson  Electron側から受け取ったリクエスト文字列(JSON形式)
   *   {
   *     xcmdno: 14,
   *     xargv0: "sqlite3ファイルパス"
   *
   * @return Electron側に応答すべきJSON文字列
   *     xijosnを引き継いだ上に、メンバーを追加する
   *     xstats:  "OK" or "NG"
   *     xerrno   "OK":0, "NG":非０
   *     xmesag   "OK":"", "NG":エラーメッセージ
   *   }
   */
  #fcmd14( xijson) {
    let xsqldb = xijson.xargv0;

    xsqldb = path.join( process.cwd() + "\\data", xsqldb);

    if ( fs.existsSync(xsqldb)) {
      fs.unlinkSync(xsqldb);
    }

    try {
      const db = new sqlite3.Database(xsqldb);
      db.close();

      let xojson = {
        xcmdno: 14,
        xargv0: xijson.xargv0,
        xstats: "OK",
        xerrno: 0,
        xmesag: ""
      };
      ...
      let xojson = {
        ...
        xstats: "NG",
        xerrno: 8,
        xmesag: this.#fermsg(8) + xsqldb + e.message
      };
      return JSON.stringify(xojson);
    }
  }
   ...
   *     xargv2: 配列
   *   }
   * @param[out]   xojson JSON応答
   *  {
   *  I  xcmdno: 15,
   *  I  xargv0: "sqlite3ファイルパス"
   *  I  xargv1: クエリ文
   *  I  xargv2: 配列
   *  O  xstats:  "OK" or "NG"
   *  O  xerrno   "OK":0, "NG":非０
   *  O  xmesag   "OK":"", "NG":エラーメッセージ
   *  }
   * @return xojson
   */
  #fcmd15( xijson) {
    let   xsqldb =    xijson.xargv0;
    const xtmsql =    xijson.xargv1;
    const xparam =    xijson.xargv2;
                                        //-----------------
                                        // SQLiteオープン
                                        //-----------------
    xsqldb = path.join( process.cwd() + "\\data", xsqldb);
    try {
      ...
      db.serialize(() => {
        db.run( xtmsql, ... xparam);
      });
    } catch (e) {
      let xojson = {
        xcmdno: 15,
        xargv0: xijson.xargv0,
        xargv1: xijson.xargv1,
        xargv2: xijson.xargv2,
        xstats: "OK",
        xerrno: 8,
        xmesag: this.#fermsg(8) + xsqldb + e.message
      };
   ...
   * @return xojson
   */
  #fcmd16_await( xsqldb, xtmsql, xparam) {
    return new Promise( (resolv, reject) => {
      const db = new sqlite3.Database(xsqldb);
      db.all( xtmsql, xparam, (err, rows) => {
...
async #fcmd16( xijson) {
    ...
    xsqldb = path.join( process.cwd() + "\\data", xsqldb);
    try {
      xreslt = await this.#fcmd16_await( xsqldb, xtmsql, xparam);
    } catch (e) {
      let xojson = {
        xcmdno: 16,
        xargv0: xijson.xargv0,
  ...
  fentry( xinbuf) {
    ...
    try {
      ...
      if ( xijson.xcmdno == 1) {
        if ( ! ("xdllnm" in xijson)) {
          throw new Error("xdllnm not found.");
        }
        if ( ! ("xfuncs" in xijson)) {
          throw new Error("xfuncs not found.");
        }
        ...
        if ( ! ("xlibno" in xijson)) {
          throw new Error("xlibno not found.");
        }
        if ( ! ("xfuncn" in xijson)) {
          throw new Error("xfuncn not found.");
        }
        if ( ! ("xinarg" in xijson)) {
          throw new Error("xinarg not found.");
        }
        const xojson = this.#fcmd02( xijson);
        console.log( xojson);
        return xojson;

      } else if ( xijson.xcmdno == 3) {
        const xojson = this.#fcmd03( xijson);
        console.log( xojson);
        return xojson;

      } else if ( xijson.xcmdno == 14) {
        if ( ! ("xargv0" in xijson)) {
          throw new Error("xargv0 not found.");
        }
        if ( ! ("xargv1" in xijson)) {
          throw new Error("xargv1 not found.");
        }
        if ( ! ("xargv2" in xijson)) {
          throw new Error("xargv2 not found.");
        }
        const xojson = this.#fcmd14( xijson);
        console.log( xojson);
        return xojson;
        
      } else if ( xijson.xcmdno == 15) {
        if ( ! ("xargv0" in xijson)) {
          throw new Error("xargv0 not found.");
        }
        if ( ! ("xargv1" in xijson)) {
